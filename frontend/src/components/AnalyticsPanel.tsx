...
import { analyticsTrades, type AnalyticsTrade } from '../data/mockData'
...
export function AnalyticsPanel() {
  ...
  const symbols = useMemo(
    () => ['all', ...new Set(analyticsTrades.map((trade) => trade.symbol))],
    [],
  )
  const sessions = useMemo(
    () => ['all', ...new Set(analyticsTrades.map((trade) => trade.session))],
    [],
  )
  ...
  const summary = useMemo(() => {
    ...
    const net = filteredTrades.reduce((sum, trade) => sum + trade.pnl, 0)
    const grossProfit = wins.reduce((sum, trade) => sum + trade.pnl, 0)
    const grossLoss = Math.abs(losses.reduce((sum, trade) => sum + trade.pnl, 0))
    const bestTrade = filteredTrades.reduce((best, trade) => (trade.pnl > best ? trade.pnl : best), 0)
    const worstTrade = filteredTrades.reduce(
      (worst, trade) => (trade.pnl < worst ? trade.pnl : worst),
      0,
    )
  ...
  const longShortBreakdown = useMemo(() => {
    return ['Long', 'Short'].map((side) => ({
      ...
      count: filteredTrades.filter((trade) => trade.side === side).length,
    }))
  }, [filteredTrades])
    ...
    <div className="screen-stack">
        ...
        <div className="analytics-filter-grid">
          ...
          <label className="settings-field">
            ...
            <select
              ...
              onChange={(event) => updateFilter('result', event.target.value as FilterState['result'])}
            >
              <option value="all">All</option>
      ...
      <section className="analytics-summary-grid">
        <article className="metric-card">
          ...
          <strong className={`metric-value ${summary.net >= 0 ? 'positive' : 'negative'}`}>
      ...
      <section className="analytics-content-grid">
        <article className="panel analytics-card">
          <div className="panel-heading">
            <div>
              ...
              <p>Performance grouped by trading session, mirroring the prototype analytics view.</p>
            </div>
          </div>
            ...
            {sessionBreakdown.map((item) => (
              <div className="analytics-bar-row" key={item.label}>
                <div className="analytics-bar-header">
                  ...
                  <strong className={item.value >= 0 ? 'positive' : 'negative'}>
        ...
        <article className="panel analytics-card">
          <div className="panel-heading">
            <div>
              <h3>P&amp;L by symbol</h3>
              <p>Top contributors in the filtered set.</p>
            </div>
          </div>
          <div className="analytics-bar-list">
        ...
        <article className="panel analytics-card">
          <div className="panel-heading">
            <div>
              <h3>Long vs Short</h3>
              <p>Keep directional performance clearly separated.</p>
            </div>
          </div>
          <div className="analytics-bar-list">
            {longShortBreakdown.map((item) => (
              <div className="analytics-bar-row" key={item.label}>
                <div className="analytics-bar-header">
                  <span>
                    {item.label} · {item.count} trades
                  </span>
                  <strong className={item.value >= 0 ? 'positive' : 'negative'}>
                    {formatCurrency(item.value)}
                  </strong>
        ...
        <article className="panel analytics-card">
          <div className="panel-heading">
            <div>
              ...
              <p>Daily rhythm overview from the current filters.</p>
            </div>
          </div>
          <div className="analytics-bar-list">
